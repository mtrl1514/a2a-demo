...
class ResearchAgent:
    def __init__(self):
        ...
        self.llm = ChatBedrock(
            ...
            region_name=os.getenv("BEDROCK_REGION", "ap-northeast-2"),
            model_kwargs={
                "temperature": 0.7,
                "max_tokens": 4000
            }
...
port = int(os.getenv("RESEARCH_PORT", 9101))
...
def main():
    ...
    try:
        ...
        if credentials is None:
            print("[WARNING]  Warning: AWS credentials not found!")
            print("   Configure with: aws configure")
        else:
            print("[OK] AWS credentials found")
    except Exception as e:
        print(f"[WARNING]  AWS credentials check failed: {e}")
    ...
    print(f"[START] Starting Research Agent (LangGraph + A2A) on http://localhost:{port}")
